...
- `GET /reports/summary?from=YYYY-MM-DD&to=YYYY-MM-DD&comparePrior=true|false` — custom date-range reporting (web-only consumer). Returns KPIs (income, expenses, net, extra spent, counts, avg daily spend), `byTag`, `subscriptionSplit`, time-series buckets (day/week/month auto granularity), optional equal-length prior-period KPI comparison, and top-5 budgets by spend. Actual persisted expenses/income only (no projected rows). Max range 730 days. Bypasses moka cache (like date-filtered `GET /expenses`).
