...
- **`POST /budgets/:id/expenses`:** any valid expense `date` (no pay-period gate). No remaining-budget cap on write — tracking only. Projection / period-view display still uses budget `start_date`/`end_date` for when the envelope appears (`budget_status`, `expense_period`).
